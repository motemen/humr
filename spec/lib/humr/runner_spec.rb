...
describe Humr::Runner do
  ...
  describe '#readable_line' do
    it 'handles accesslog' do
      ...
      INPUT
        66.249.66.33 - - [#{Time.parse('2013-09-08 04:33:45 +0900').to_s.color(:yellow)}] "GET /randompoku/?#{'クックパッド'.color(:green)} HTTP/1.1" 200 #{'4.7k'.color(:cyan)} "-" "#{'Googlebot/2.1'.color(:magenta)}"
      READABLE
    end
  end
end
